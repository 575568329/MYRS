...
#### 🎉 新增功能
- **Epic 免费游戏** - 新增游戏平台，展示每周免费游戏，支持封面图展示
- **金融/股票板块** - 新增3个股票平台：热门股票、上证指数、科技股
  - 实时股票行情数据（价格、涨跌幅、成交量、成交额）
  - 支持15只热门股票/指数同时查询
  - 数据缓存1分钟，确保实时性
- **游戏平台增强** - 新增崩坏3、星穹铁道等游戏热搜

#### ⚡ 性能优化
- **股票数据优化** - 使用腾讯股票API，一次请求获取多只股票数据
- **重试机制** - 股票API失败时自动重试，提升数据获取成功率
- **超时配置** - 股票平台超时设置为25秒，适应多股票查询需求

#### 🔧 技术改进
- **数据格式解析** - 兼容腾讯和新浪两种股票数据格式
- **字符编码修复** - 股票名称使用配置的中文，避免API返回的乱码问题
- **字段映射修正** - 修复成交量和成交额字段映射错误
- **错误处理增强** - 完善各种网络错误的提示信息

#### 📋 配置变更
```javascript
// src/config.js 新增配置
PLATFORM_TIMEOUT: {
  'stock-hot': 25000,    // 热门股票 25 秒
  'stock-sh': 25000,     // 上证指数 25 秒
  'stock-tech': 25000    // 科技股 25 秒
}
```
...
- **🔥 多平台聚合** - 支持43+主流平台热搜，一键切换
- **📂 分类浏览** - 综合类、社交类、视频类、资讯类、科技类、娱乐类、游戏类、阅读类、金融类、艺术类
...
API: {
  ...
  PLATFORM_TIMEOUT: {          // 平台专属超时配置
    ...
    'stock-hot': 25000,      // 热门股票 25 秒（一次请求多个股票）
    'stock-sh': 25000,       // 上证指数 25 秒（一次请求多个股票）
    'stock-tech': 25000,     // 科技股 25 秒（一次请求多个股票）
    'epic-free': 5000,       // Epic免费游戏 5 秒
    'artic': 10000,          // 芝加哥艺术学院 10 秒
    'metmuseum': 15000       // 大都会博物馆 15 秒
  },
...
- 数据来源：腾讯股票API
- 支持的股票：
  - 热门股票：贵州茅台、五粮液、中国平安等15只热门股
  - 上证指数：上证指数及权重股
  - 科技股：中兴通讯、海康威视、东方财富等10只科技股
...
- 在游戏分类中选择 **"Epic免费游戏"**
- 显示当前免费的游戏和即将免费的游戏
- 点击游戏封面可跳转到 Epic Store 领取
- 每周周四晚上 11 点（PT时间）更新免费游戏
